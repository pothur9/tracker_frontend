...
export function SeatSelection({ 
  ...
  const handleSeatClick = (seatId: string, isAvailable: boolean, isBlocked: boolean) => {
    ...
    onSeatSelect(seatId); // Pass the seat ID to parent for handling
  };

  useEffect(() => {
    const fetchSeatData = async () => {
      try {
        const flightSearchResults = JSON.parse(localStorage.getItem("flightSearchResults") || "{}");
        traceId = flightSearchResults?.Response?.TraceId;
        console.log("saddddddddddddddddddddddddddddddddddddd",traceId);
        console.log("Result Index:", resultIndex);

        if (resultIndex === undefined) {
          throw new Error("resultIndex is undefined");
        }
        console.log("Fetching seat data with:", { traceId, resultIndex: resultIndex.toString() });
        ...
        const seatData = response.Response.SeatDynamic.flatMap((segment: Segment) => 
        ...
        const isOneStopFlight = selectedFlight?.outboundFlight?.FareRules?.length > 1;
        setIsOneStop( isonestop);
        console.log("0111111111111111", isonestop)

        if (isOneStopFlight) {
          const groupedFlights = seatData.reduce(
            (acc, seat) => {
              if (!acc.firstFlight.length || acc.firstFlight[0].FlightNumber === seat.FlightNumber) {
