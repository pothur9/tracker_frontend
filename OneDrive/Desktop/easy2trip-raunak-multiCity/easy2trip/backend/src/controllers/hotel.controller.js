...
exports.searchHotels = async (req, res) => {
  ...
  try {
    ...
    const apiUrl = "https://affiliate.tektravels.com/HotelAPI/Search";

    let allHotelResults = [];

    const fetchHotels = async (hotelCodeChunk) => {
      try {
        console.log(
          "hotelCodeChunk",
          JSON.stringify({
    ...
    res.json({
      ...
      totalPages: Math.ceil(allHotelResults.length / limit),
      hotels: mergedHotels,
    });
  } catch (error) {
    console.error(error);
...
exports.getHotelRoom = async (req, res) => {
  ...
  try {
    if ((!bookingCode, !resultIndex, !hotelCode, !traceId)) {
      return res.status(400).json({
        message: "Invalid input. Please provide all required fields.",
      });
    }
    ...
    const apiUrl =
      "https://api.tektravels.com/BookingEngineService_Hotel/hotelservice.svc/rest/GetHotelRoom";
...
exports.hotelBook = async (req, res) => {
  const { bookingCode, guestNationality, netAmount, hotelPassenger } = req.body;

  try {
    if ((!bookingCode, !guestNationality, !netAmount, !hotelPassenger)) {
      return res.status(400).json({
        message: "Invalid input. Please provide all required fields.",
      });
    }
