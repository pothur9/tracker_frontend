...
export function PassengerDetails({
  ...
  formData,
  onFormChange,
  pessangerData = { adults: 1, children: 0, infants: 0 },
  maxChildren = 0,
  maxInfants = 0
}: PassengerDetailsProps) {
  // Safe access to passenger counts with fallbacks
  const adultCount = pessangerData?.adults ?? maxPassengers ?? 1;
  const childCount = pessangerData?.children ?? maxChildren ?? 0;
  const infantCount = pessangerData?.infants ?? maxInfants ?? 0;
  ...
  const handleInfantChange = (
    index: number,
    field: keyof InfantDetail | 'dob.day' | 'dob.month' | 'dob.year',
    value: string
  ) => {
    const updatedInfants = [...formData.infants];
    const currentInfant = { ...updatedInfants[index] };

    if (field === 'dob.day' || field === 'dob.month' || field === 'dob.year') {
      const dobField = field.split('.')[1] as keyof typeof currentInfant.dob;
      currentInfant.dob = {
        ...currentInfant.dob,
        [dobField]: value
      };
    } else {
      (currentInfant as any)[field] = value;
    }

    updatedInfants[index] = currentInfant;
    onFormChange("infants", updatedInfants);
  };

  const addPassenger = () => {
    if (formData.adults.length >= adultCount) return;
  ...
  const addChild = () => {
    if (formData.children.length >= childCount) return;
  ...
  const addInfant = () => {
    if (formData.infants.length >= infantCount) return;

    const newInfant: InfantDetail = {
      firstName: "",
      lastName: "",
      sex: "",
      dob: { day: "1", month: "Jan", year: "2025" },
    };
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {formData.adults.length}/{adultCount} added
            </p>
          </div>
          {formData.adults.length < adultCount && (
            <Button
              variant="ghost"
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {formData.children.length}/{childCount} added
              </p>
            </div>
            {formData.children.length < childCount && (
              <Button
                variant="ghost"
                className="text-blue-600"
                onClick={addChild}
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {formData.infants.length}/{infantCount} added
              </p>
            </div>
            {formData.infants.length < infantCount && (
              <Button
                variant="ghost"
                className="text-blue-600"
                onClick={addInfant}
      ...
      <Card className="p-6">
        <div className="flex items-center gap-2 mb-4">
          <Checkbox
            ...
            onCheckedChange={(checked) =>
              onFormChange("gstNumber", checked ? "" : null)
            }
          />
